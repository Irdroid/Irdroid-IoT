...
import com.microcontrollerbg.irdroid.cloud.ColorTool;
import com.microcontrollerbg.irdroid.cloud.ActionListener.Action;
import com.microcontrollerbg.irdroid.cloud.R.color;
...
number=0;
      ...
      int currentResource = (tagNum == null) ? R.drawable.mqtt_default : tagNum.intValue ();

      // Now that we know the current resource being displayed we can handle the DOWN and UP events.

      switch (action) {
      case MotionEvent.ACTION_DOWN :
         if (currentResource == R.drawable.mqtt_default) {
...
myVib.vibrate(20);
     ...
     // 	   mCIR.transmit(38000, plus);     
         ...
         if (ct.closeMatch (getResources().getColor(color.Khaki), touchColor, tolerance)) 
  	   {
        	 nextImage = R.drawable.mqtt_transmit;  
      	   myVib.vibrate(30);
      	 System.out.println(connection.getType());
      	    if (connection.getType() == 2){
      	     ...
      	     if (!connected){
      	  	 ...
      	  	 try {
      	  	      Connections.getInstance(context).getConnection(clientHandle).getClient()
      	  	          .publish(connection.getId()+"@/"+connection.getchannel()+"5", "0".getBytes(), 2, false );
      	  	    }
      	  	    catch (MqttSecurityException e) {
      	  	      Log.e(this.getClass().getCanonicalName(), "Failed to publish a messged from the client with the handle " + clientHandle, e);
      	  	    }
         ...
         if (ct.closeMatch (getResources().getColor(color.Violet), touchColor, tolerance)) 
  	   {
        	 nextImage = R.drawable.mqtt_transmit;  
  	   myVib.vibrate(30);
    ...
    if (connection.getType() == 2){
     ...
     if (!connected){
  	 ...
  	 try {
  	      Connections.getInstance(context).getConnection(clientHandle).getClient()
  	          .publish(connection.getId()+"@/"+connection.getchannel()+"5", "1".getBytes(), 2, false );
  	    }
  	    catch (MqttSecurityException e) {
  	      Log.e(this.getClass().getCanonicalName(), "Failed to publish a messged from the client with the handle " + clientHandle, e);
  	    }
