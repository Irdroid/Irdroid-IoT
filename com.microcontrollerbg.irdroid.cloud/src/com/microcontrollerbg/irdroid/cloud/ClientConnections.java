...
import com.microcontrollerbg.irdroid.cloud.ActionListener.Action;
import com.microcontrollerbg.irdroid.cloud.Connection.ConnectionStatus;
...
public class ClientConnections extends ListActivity {
   ...
   */
  final static String TOKEN = "com.microcontrollerbg.irdroid.cloud.ClientConnections";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (!firstrun){
    	  ...
    	  Intent intent = new Intent(context,OrderWizardActivity.class)
    ...
    // get all the available connections
     connections = Connections.getInstance(this)
        .getConnections();
    ...
    if (!contextualActionBarActive) {
      ...
      // selected connection
          ...
          intent.setClassName(getApplicationContext().getPackageName(),
              "com.microcontrollerbg.irdroid.cloud.ConnectionDetails");
  ...
  protected void onResume() {
    ...
    try {
      List<Connection> l = persistence.restoreConnections(this);
      for (Connection c : l) {
  ...
  private void connectAction(Bundle data) {
    ...
    String location = (String) data.get(ActivityConstants.location);
    String clientId = (String) data.get(ActivityConstants.clientId);
    String channelId = (String) data.get(ActivityConstants.channelId);
    int port = Integer.parseInt((String) data.get(ActivityConstants.port));
    int type = Integer.parseInt((String) data.get(ActivityConstants.type));
    ...
    Connection connection = new Connection(clientHandle, clientId,location, server,channelId, port,
        this, client, ssl, type);
  ...
  private class LongClickItemListener implements OnItemLongClickListener, ActionMode.Callback, OnClickListener {
    ...
    public void onDestroyActionMode(ActionMode mode) {
      ...
      selectedView.setBackgroundColor(getResources().getColor(android.R.color.white));
      selectedView = null;
      clientConnections.contextualActionBarActive = false;
